...
function MyGMRESUpdated(A::Matrix{Float64},b::Vector{Float64},x₀::Vector{Float64},tol::Float64)
    d::Int16=length(x₀)
    ...
    T[it,it] = V[:,1]'*A*V[:,1]
    #println("T :",T)
    Qₙ::Matrix{Float64}, Rₙ::Matrix{Float64} = LinearAlgebra.qr(T)
    ...
    while norm_r >= tol 
        Vₙ = V #(it,it)
        V = HermitianLanczosUpdated(A,V,it) #(d,it+1)
        T = V'*A*Vₙ #(it+1, it)
        ...
        Hₙ = Tₙ*Qₙ' #(it,it)
        γₙ::Float64 = sqrt(Hₙ[it,it]^2 + T[it+1,it]^2)
        cₙ::Float64 = Hₙ[it,it]/γₙ  ; sₙ::Float64= T[it+1,it]/γₙ ;
        Cₙ = [cₙ sₙ ; sₙ -cₙ] #(2,2)
        if it == 1 
            Gₙ = Cₙ
        else 
        ...
        Qₙ = Gₙ*[Qₙ zeros(it) ; zeros(it)' 1 ]  #(it+1, it+1)
            ...
            #println("Qₙ:", Qₙ)
        #end 
        @assert isapprox((Qₙ')*Qₙ, I;atol=1e-4) "Qn doit être orthogonale"

        Pₙ::Matrix{Float64} = Qₙ*T #(it+1,it)
        Rₙ = [I(it) zeros(it)]*Pₙ #(it, it)
...
function HermitianLanczosUpdated(A::Matrix{Float64},V::Matrix{Float64},it::Int16) #To Work on
